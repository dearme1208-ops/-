  ...
  return (
    <div className="space-y-4">
      ...
      <div className="panel space-y-3 p-4">
        ...
        {visualMode === "powerpro" && (
          <p className="text-xs text-cream/50">
            「ToDo」タブと「案件」タブにも同じ演出が及びます。ToDoの各項目はスカウト候補の選手として胸像が描かれ（期日が迫るほど背景が熱を帯び、集中線が入ります）、期日までの余裕と重要度からS〜Cの評価が付きます。案件には段階（マイルストーン）を1試合ずつに見立てた電光掲示板風の勝敗表が表示され、完了＝○、期日超過＝●、未消化＝空きマスとして並び、勝率から順位（首位・Aクラスなど）が決まります。
          </p>
        )}
